...
<html>
    <head>
        ...
        <style>
            ...
            .box a{height:170px;overflow:hidden; border-bottom:#d6d6d6 1px solid;width:auto; display:block; line-height:3px; padding:10px 8px 0 0}
                    ...
                    <div id="divjrzd" class="mod">
                        <div id="ulist" class="box">
                            <c:forEach items="${pu.list}" var="map">
                                <!--特殊区域 客户案例无下级-->
                                <c:if test="${9==sid}">
                                    <div><a href="javascript:void(0);"> <img src="${map.img}" /> <i>${map.name}</i> </a></div>
                                        </c:if>
                                        <c:if test="${9!=sid}">

                                    <div><a href="${pageContext.servletContext.contextPath}/ForeServlet?method=xiangqing&id=${map.id}"> <img src="${map.img}" /> <i>${map.name}</i> </a> </div>
