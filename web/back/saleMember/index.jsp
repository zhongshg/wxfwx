            ...
            <div id="body-wrapper">
                ...
                <div class="content-box">
                    ...
                    <div class="content-box-content">
                        <form id="form1" method="post" action="${pageContext.servletContext.contextPath}/servlet/SaleMemberServlet?method=getList">
                            <input type="hidden" id="ids" name="ids"/>
                            openid:<input type="text" id="openid" name="openid"  value="${openid}"/>
                            是否推广：<input type="radio" name="issm" value="" checked="true"/>全部&nbsp;<input type="radio" name="issm" value="0" ${"0"==issm?"checked=\"true\"":""}/>否&nbsp;<input type="radio" name="issm" value="1" ${"1"==issm?"checked=\"true\"":""}/>是
...
<!--                            <div class="notification attention png_bg"> <a href="#" class="close"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/cross_grey_small.png" title="Close this notification" alt="close" /></a>
                                ...
                                <tbody>
                                    <c:forEach items="${pu.list}" var="subscriber" varStatus="s">
                                        <tr>
                                            ...
                                            <td>${subscriber.subscriberpromote}</td>
                                            <td>${subscriber.nosubscriberpromote}</td>
                                            <td>
                                                <c:if test="${'1'==subscriber.issm}">
                                                    是&nbsp;&nbsp;&nbsp;<a href="javascript:issm('${subscriber.openid}','0','${pu.page}');">取消</a>
                                                </c:if>
                                                <c:if test="${'0'==subscriber.issm}">
                                                    否&nbsp;&nbsp;&nbsp;<a href="javascript:issm('${subscriber.openid}','1','${pu.page}');">成为</a>
                                                </c:if>
                                            </td>
                                            <td><a href="${pageContext.servletContext.contextPath}/servlet/SaleMemberServlet?method=subscriberdetaillist&openid=${subscriber.openid}&subscriberdate=${subscriberdate}">点击查看</a></td>
                                        </tr>
                                    </c:forEach>
                                </tbody>
                            </table>
                        ...
                        <script>
            function issm(openid, issm, page) {
                if (confirm("确认？")) {
                    window.location.href = "${pageContext.servletContext.contextPath}/servlet/SaleMemberServlet?method=issm&openid=" + openid + "&issm=" + issm + "&page=" + page;
                }
            }
                        </script>
