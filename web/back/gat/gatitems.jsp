    ...
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#exeall").click(function() {
                    ...
                    if ("delall" == dropdown) {
                        if (confirm("不可恢复，确认？")) {
                            $("input[name=gatitemsckb]:checked").each(function() {
                                ids += "," + $(this).val();
                            });
                        }
                    }
                    ...
                    form1.action = "${pageContext.servletContext.contextPath}/servlet/GatItemsServlet?method=delAll";
                    form1.submit();
                });
            });
        </script>
        ...
        <jsp:include page="../../public/css.jsp"></jsp:include>
            <script type="text/javascript">
                function del(id, sid) {
                    if (confirm("不可恢复,确认？")) {
                        window.location.href = "${pageContext.servletContext.contextPath}/servlet/GatItemsServlet?method=delete&id=" + id + "&sid=" + sid;
                    }
                }
        </script>
    </head>
        ...
        <div id="body-wrapper">
            <div id="main-content">
                ...
                <ul class="shortcut-buttons-set">
                    <li><a class="shortcut-button" href="${pageContext.servletContext.contextPath}/servlet/GatItemsServlet?method=initManage&id=0&sid=${sid}"><span> 
...
<!--                            <div class="notification attention png_bg"> <a href="#" class="close"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/cross_grey_small.png" title="Close this notification" alt="close" /></a>
                            ...
                            <table>
                                ...
                                <tbody>
                                    <c:forEach items="${pu.list}" var="gatitems">
                                        <tr>
                                            <td>
                                                <input type="checkbox" name="gatitemsckb" value="${gatitems.id}"/>
                                            </td>
                                            <c:if test="${1!=sid}">
                                                <td>${gatitems.name}</td>
                                            ...
                                            <c:if test="${1==sid}">
                                                <td>${null!=gatitems.nickname?gatitems.nickname:"微平台"}</td>
                                                <td>${gatitems.praisecount}</td>
                                            </c:if>
                                            <td>
                                                <c:if test="${1==sid}">
                                                    <a href="${pageContext.servletContext.contextPath}/servlet/ForumServlet?method=getList&id=${gatitems.id}&sid=${param.forumsid}">赞</a>
                                                    <a href="${pageContext.servletContext.contextPath}/servlet/DiscussServlet?method=getList&id=${gatitems.id}">评论</a>
                                                </c:if>
                                                <!-- Icons -->
                                                <a href="${pageContext.servletContext.contextPath}/servlet/GatItemsServlet?method=initManage&id=${gatitems.id}&sid=${sid}&page=${pu.page}" title="Edit"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/pencil.png" alt="Edit" />编辑</a> 
                                                <a href="javascript:del('${gatitems.id}','${sid}');" title="Delete"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/cross.png" alt="Delete" />删除</a> 
