        ...
        <body>
            <div id="body-wrapper">
                ...
                <div class="content-box">
                    ...
                    <div class="content-box-content">
                        <div class="tab-content default-tab" id="tab2">
                            <form action="${pageContext.servletContext.contextPath}/servlet/GatItemsServlet?method=manage" id="form1" method="post" onsubmit="return test();">
                                <input type="hidden" id="id" name="id" value="${gatitems.id}"/>
                                <input type="hidden" id="sid" name="sid" value="${gatitems.sid}"/>
                                <input type="hidden" id="img" name="img" value="${gatitems.img}"/><%--图片路径--%>
                                <input type="hidden" id="state" name="state" value="${gatitems.state}"/>
                                <input type="hidden" name="page" value="${param.page}"/>
                                <input type="hidden" name="orders" value="${gatitems.orders}"/>
                                <fieldset>
                                    <!-- Set class to "column-left" or "column-right" on fieldsets to divide the form into columns -->
                                    <c:if test="${1!=gatitems.sid}">
                                        <p>
                                            <label>标题</label>
                                            <input class="text-input large-input datepicker" type="text" id="name" name="name" value="${gatitems.name}"/>
                                            <span id="namespan" class="input-notification error png_bg"></span></p>
                                        <p>
                                            <label>图片上传</label>
                                            <iframe src="${pageContext.servletContext.contextPath}/public/upload.jsp?oldimg=${gatitems.img}" class="text-input medium-input" width="300" height="26" frameborder="0" marginheight="0" marginwidth="0" scrolling="no"></iframe>
                                        </p>
                                        <p>
                                            <label>图片预览</label>
                                            <div id="result" name="result">
                                                <c:if test="${!empty gatitems.img}">
                                                    <img src='${pageContext.servletContext.contextPath}${gatitems.img}' width='200' height='200'/>
                                                </c:if>
                                                <c:if test="${empty gatitems.img}">
                                                    请上传图片
                                                </c:if>
                                    ...
                                    <p>
                                        <label>描述</label>
                                        <textarea class="text-input" id="describer" name="describer" cols="79" rows="15">${gatitems.describer}</textarea>
                ...
                KindEditor.ready(function(K) {
                    K.create('textarea[name="describer"]', {
