                        ...
                        <div id="ulist" class="box"> 
                            <c:forEach items="${pu.list}" var="newstypes">
                                <c:if test="${0==newstypes.sign}" var="flag">
                                    <a href="${pageContext.servletContext.contextPath}/ForeServlet?method=liebiao&&sid=${newstypes.sid}&id=${newstypes.id}"> <img src="${newstypes.img}" /> <i>${newstypes.name}</i> </a>
                                    </c:if>
                                    <c:if test="${!flag}">
                                    <a href="${pageContext.servletContext.contextPath}/ForeServlet?method=newstypesliebiao&sid=${newstypes.sid}&id=${newstypes.id}"> <img src="${newstypes.img}" /> <i>${newstypes.name}</i> </a>
                                    </c:if>
                                </c:forEach>
