    ...
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#exeall").click(function() {
                    ...
                    if ("delall" == dropdown) {
                        if (confirm("不可恢复，确认？")) {
                            $("input[name=gatckb]:checked").each(function() {
                                ids += "," + $(this).val();
                            });
                        }
                    }
                    ...
                    form1.action = "${pageContext.servletContext.contextPath}/servlet/GatServlet?method=delAll";
                    form1.submit();
                });
            });
        </script>
        ...
        <jsp:include page="../../public/css.jsp"></jsp:include>
            <script type="text/javascript">
                function del(id, sid) {
                    if (confirm("不可恢复,确认？")) {
                        window.location.href = "${pageContext.servletContext.contextPath}/servlet/GatServlet?method=delete&id=" + id + "&sid=" + sid;
                    }
                }
        </script>
    </head>
        ...
        <div id="body-wrapper">
            <div id="main-content">
                ...
                <ul class="shortcut-buttons-set">
                    <li><a class="shortcut-button" href="${pageContext.servletContext.contextPath}/servlet/GatServlet?method=initManage&id=0&sid=${sid}"><span> 
...
<!--                            <div class="notification attention png_bg"> <a href="#" class="close"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/cross_grey_small.png" title="Close this notification" alt="close" /></a>
                            ...
                            <table>
                                ...
                                <tbody>
                                    <c:forEach items="${pu.list}" var="gat">
                                        <tr>
                                            <td>
                                                <input type="checkbox" name="gatckb" value="${gat.id}"/>
                                            </td>
                                            <td>${gat.name}</td>
                                            <td><img src="${pageContext.servletContext.contextPath}${gat.img}" width="60px" height="40px"/></td>
                                            <td>${gat.remark}</td>
                                            <td>
                                                <!-- Icons -->
                                                <a href="${pageContext.servletContext.contextPath}/servlet/GatServlet?method=initManage&id=${gat.id}&sid=${sid}&page=${pu.page}" title="Edit"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/pencil.png" alt="Edit" />编辑</a> 
                                                <a href="javascript:del('${gat.id}','${sid}');" title="Delete"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/cross.png" alt="Delete" />删除</a> 
                                                <a href="${pageContext.servletContext.contextPath}/servlet/GatServlet?method=initSelect&id=${gat.id}&sid=${sid}&page=${pu.page}" title="Edit Meta"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/hammer_screwdriver.png" alt="Edit Meta" />设置明细</a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                </tbody>
