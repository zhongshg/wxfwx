    ...
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#exeall").click(function() {
                    ...
                    if ("delall" == dropdown) {
                        if (confirm("不可恢复，确认？")) {
                            $("input[name=powersckb]:checked").each(function() {
                                ids += "," + $(this).val();
                            });
                        }
                    }
                    ...
                    form1.action = "${pageContext.servletContext.contextPath}/servlet/PowersServlet?method=delAll";
                    form1.submit();
                });
            });
        </script>
        ...
        <jsp:include page="../public/css.jsp"></jsp:include>
            <script type="text/javascript">
                function del(id) {
                    if (confirm("不可恢复,确认？")) {
                        window.location.href = "${pageContext.servletContext.contextPath}/servlet/PowersServlet?method=delete&id=" + id;
                    }
                }
        </script>
    </head>
...
<!--                            <div class="notification attention png_bg"> <a href="#" class="close"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/cross_grey_small.png" title="Close this notification" alt="close" /></a>
                            ...
                            <table>
                                ...
                                <tbody>
                                    <c:forEach items="${pu.list}" var="powers">
                                        <tr>
                                            <td>
                                                <input type="checkbox" name="powersckb" value="${powers.id}"/>
                                            </td>
                                            <td>${powers.name}</td>
                                            <td>${powers.parentname}</td>
                                            <td>${powers.url}</td>
                                            <td>${powers.remark}</td>
                                            <td>
                                                <!-- Icons -->
                                                <a href="${pageContext.servletContext.contextPath}/servlet/PowersServlet?method=initManage&id=${powers.id}&page=${pu.page}" title="Edit"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/pencil.png" alt="Edit" />编辑</a> 
                                                <a href="javascript:del('${powers.id}');" title="Delete"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/cross.png" alt="Delete" />删除</a> 
