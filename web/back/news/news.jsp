    ...
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#exeall").click(function() {
                    ...
                    if ("delall" == dropdown) {
                        if (confirm("不可恢复，确认？")) {
                            $("input[name=newsckb]:checked").each(function() {
                                ids += "," + $(this).val();
                            });
                        }
                    }
                    ...
                    form1.action = "${pageContext.servletContext.contextPath}/servlet/NewsServlet?method=delAll";
                    form1.submit();
                });
            });
        </script>
        ...
        <jsp:include page="../../public/css.jsp"></jsp:include>
            <script type="text/javascript">
                function del(id, sid) {
                    if (confirm("不可恢复,确认？")) {
                        window.location.href = "${pageContext.servletContext.contextPath}/servlet/NewsServlet?method=delete&id=" + id + "&sid=" + sid;
                    }
                }
        </script>
    </head>
        ...
        <div id="body-wrapper">
            <div id="main-content">
                ...
                <ul class="shortcut-buttons-set">
                    <li><a class="shortcut-button" href="${pageContext.servletContext.contextPath}/servlet/NewsServlet?method=initManage&id=0&sid=${sid}"><span> 
                ...
                <div class="content-box">
                    ...
                    <div class="content-box-content">
                        <form id="form1" method="post">
                            ...
                            分类:<select name="stypeid">
                                <option value="0">全部</option>
                                <c:forEach items="${newstypeslist}" var="newstypes">
                                    <option value="${newstypes.id}" ${stypeid==newstypes.id?"selected='selected'":""}>${newstypes.name}</option>
                                </c:forEach>
                            </select>
...
<!--                            <div class="notification attention png_bg"> <a href="#" class="close"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/cross_grey_small.png" title="Close this notification" alt="close" /></a>
                            ...
                            <table>
                                ...
                                <tbody>
                                    <c:forEach items="${pu.list}" var="news">
                                        <tr>
                                            <td>
                                                <input type="checkbox" name="newsckb" value="${news.id}"/>
                                            </td>
                                            <td>${news.name}</td>
                                            <td>${news.author}</td>
                                            <!--<td>${news.source}</td>-->
                                            <td><img src="${pageContext.servletContext.contextPath}${news.img}" width="60px" height="40px"/></td>
                                            <td>${news.typename}</td>
                                            <td>${news.times}</td>
                                            <td>${news.viewcounts}</td>
                                            <td>
                                                <!-- Icons -->
                                                <a href="${pageContext.servletContext.contextPath}/servlet/NewsServlet?method=initManage&id=${news.id}&page=${pu.page}&sid=${sid}" title="Edit"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/pencil.png" alt="Edit" />编辑</a> 
                                                <a href="javascript:del('${news.id}','${sid}');" title="Delete"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/cross.png" alt="Delete" />删除</a> 
