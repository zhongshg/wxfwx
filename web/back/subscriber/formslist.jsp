        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#exeall").click(function() {
                    ...
                    if ("delall" == dropdown) {
                        if (confirm("不可恢复，确认？")) {
                            $("input[name=formvalueckb]:checked").each(function() {
                                ids += "," + $(this).val();
                            });
                        }
                    }
                    ...
                    form1.action = "${pageContext.servletContext.contextPath}/servlet/FormvaluesServlet?method=delAll";
                    form1.submit();
                });
            });
        </script>
...
<!--                            <div class="notification attention png_bg"> <a href="#" class="close"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/cross_grey_small.png" title="Close this notification" alt="close" /></a>
                            ...
                            <table>
                                <c:forEach items="${pu.list}" var="form">
                                    <thead>
                                        <tr>
                                            <th>
                                                ${form.name}:
                                        ...
                                        <tr>
                                            ...
                                            <c:forEach items="${form.list}" var="element">
                                                <th>${element.name}</th>
                                                </c:forEach>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <c:forEach items="${form.list[0].list}" var="fv" varStatus="s">
                                            <tr>
                                                <td>
                                                    <input type="checkbox" name="formvalueckb" value="${form.id}---${fv.keyss}"/>
                                                </td>
                                                <c:forEach items="${form.list}" var="element">
                                                    <c:forEach items="${element.list}" var="formvalue" begin="${s.index}" end="${s.index}">
                                                        <td>${formvalue.valuess}</td>
                                                    </c:forEach>
                                                </c:forEach>
