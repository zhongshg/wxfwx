        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#exeall").click(function() {
                    ...
                    if ("delall" == dropdown) {
                        if (confirm("不可恢复，确认？")) {
                            $("input[name=openprizeckb]:checked").each(function() {
                                ids += "," + $(this).val();
                            });
                        }
                    }
                    ...
                    form1.action = "${pageContext.servletContext.contextPath}/servlet/OpenprizeServlet?method=delAll";
                    form1.submit();
                });
            });
            function exp(id) {
                var form1 = document.getElementById("form1");
                form1.action = "${pageContext.servletContext.contextPath}/servlet/OpenprizeServlet?method=exp&id=" + id;
                form1.submit();
            }
        </script>
        <!--                       CSS                       -->
                ...
                <div class="content-box">
                    ...
                    <div class="content-box-content">
                        <form id="form1" method="post">
                            ...
                            <input type="hidden" name="sign" value="${pu.sign}"/>
                            <input type="hidden" name="page" value="${pu.page}"/>
                        </form>
...
<!--                            <div class="notification attention png_bg"> <a href="#" class="close"><img src="${pageContext.servletContext.contextPath}/resources/images/icons/cross_grey_small.png" title="Close this notification" alt="close" /></a>
                            ...
                            <table>
                                ...
                                <tbody>
                                    <c:forEach items="${pu.list}" var="openprize" varStatus="s">
                                        <tr>
                                            <td>
                                                <input type="checkbox" name="openprizeckb" value="${openprize.id}"/>
                                            </td>
                                            <td>${s.count}</td>
                                            <td>${openprize.openid}</td>
                                            <td>${openprize.aname}${openprize.vname}</td>
                                            <td>${openprize.pname}${"1"==openprize.state?"58到家红包":""}</td>
                                            <td>${openprize.sn}</td>
                                            <td>${openprize.tel}</td>
                                            <td>${openprize.opendate}</td>
                                            <td>${"1"==openprize.state?"中奖":"未中奖"}</td>
<!--                                            <td>${openprize.name}</td>
                                            <td>${"1"==openprize.sex?"女":"男"}</td>-->
                                            <td>
                                                <c:if test="${'0'==openprize.state}">
                                                    未中奖
                                                </c:if>
                                                <c:if test="${'1'==openprize.state}">
                                                    <c:if test="${'1'==openprize.isexp}">
                                                        已兑奖
                                                    </c:if>
                                                    <c:if test="${'0'==openprize.isexp}">
                                                        <a href="javascript:exp('${openprize.id}');" style="text-decoration: none;">兑奖</a>
                                                    </c:if>
                                                </c:if>
                                            </td>
                                        </tr>
